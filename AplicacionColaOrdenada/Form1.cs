...
namespace AplicacionColaOrdenada
{
    ...
    {
        ClaseColaOrdenada<Personaje> colaPersonajes = new ClaseColaOrdenada<Personaje>(); //Se crea la cola de personajes
        ...
        string[] colaFotos = Directory.GetFiles(Application.StartupPath + "\\FotosPersonajes"); //Arreglo para almacenar las rutas de las imagenes
        public Form1()
        {
            InitializeComponent();
        }
        ...
        private void AgregarDatagridView() //Método para agregar los datos al DataGridView recorriendo la cola con el ciclo foreach
        {
            try
            {
                dtgDatos.Rows.Clear();
                foreach (Personaje unPersonaje in colaPersonajes)
                {
        ...
        private void btnAgregar_Click(object sender, EventArgs e)
        {
            if (ValidarOperacion("¿Está seguro de querer agregar un personaje?", MessageBoxIcon.Question))
            {
                try
                {
                    ...
                    colaPersonajes.Encolar(unPersonaje);
                    AgregarDatagridView();
                    LimpiarControles();
            ...
            if (ValidarOperacion("¿Está seguro de querer generar personajes?", MessageBoxIcon.Question))
            {
                try
                {
                    for (int i = 0; i < 10; i++)
                    {
                        ...
                        unPersonaje.RutaFoto = colaFotos[aleatorio.Next(0, 10)]; //Se establece una imagen aleatoria dependiendo el indíce dado por un número aleatorio del 0 al 9
                        colaPersonajes.Encolar(unPersonaje); //Se agrega el objeto a la cola
        ...
        private void btnBuscar_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                LimpiarControles();
                unPersonajeBuscado = colaPersonajes.BuscarNodo(unPersonajeBuscado);
                ...
                MessageBox.Show(colaPersonajes.BuscarNodo(unPersonajeBuscado).ToString(), "Personaje Encontrado", MessageBoxButtons.OK, MessageBoxIcon.Information);
            }
            catch (Exception ex)
            {
                MessageBox.Show(ex.Message, "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
        ...
        private void btnEliminar_Click(object sender, EventArgs e)
        {
            if (ValidarOperacion("¿Está seguro de querer eliminar este personaje?", MessageBoxIcon.Warning))
            {
                try
                {
                    ...
                    MessageBox.Show(colaPersonajes.BuscarNodo(unPersonajeEliminado).ToString(), "Personaje Eliminado", MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
                    colaPersonajes.Desencolar(unPersonajeEliminado);
                    AgregarDatagridView();
                    LimpiarControles();
                }
                catch (Exception ex)
        ...
        {
            if (ValidarOperacion("¿Está seguro de querer eliminar el Top?", MessageBoxIcon.Warning))
            {
                try
                {
                    Personaje unPersonajeEliminado = new Personaje();
                    MessageBox.Show(colaPersonajes.Desencolar().ToString(), "Personaje Eliminado", MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
                    AgregarDatagridView();
                    LimpiarControles();
                }
                catch (Exception ex)
        ...
        {
            if (ValidarOperacion("¿Está seguro de querer vacíar la cola?", MessageBoxIcon.Warning))
            {
                try
                {
                    colaPersonajes.Vaciar();
                    dtgDatos.Rows.Clear(); //Se limpian todas las filas del DataGridView
                    LimpiarControles();
                    MessageBox.Show("Se ha vaciado exitosamente la cola", "Cola Vaciada", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                }
                catch (Exception ex)
                {
                    MessageBox.Show(ex.Message, "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
        ...
        private void dtgDatos_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            try
            {
                ...
                picFoto.ImageLocation = colaPersonajes.BuscarNodo(unPersonajeBuscado).RutaFoto; //Se establece la localización de la imagen
                picFoto.Load(colaPersonajes.BuscarNodo(unPersonajeBuscado).RutaFoto); //Se carga la imagen 
            }
            catch (Exception ex)
            {
                MessageBox.Show(ex.Message, "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
