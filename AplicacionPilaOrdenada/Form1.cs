...
namespace AplicacionPilaOrdenada
{
    ...
    {
        ClasePilaOrdenada<Personaje> listaPersonajes = new ClasePilaOrdenada<Personaje>(); //Se crea la lista de personajes
        ...
        private void btnAgregar_Click(object sender, EventArgs e)
        {
            if (ValidarOperacion("¿Está seguro de querer agregar un personaje?", MessageBoxIcon.Question))
            {
                try
                {
                    ...
                    listaPersonajes.Push(unPersonaje);
                    AgregarDatagridView();
                    LimpiarControles();
            ...
            if (ValidarOperacion("¿Está seguro de querer generar personajes?", MessageBoxIcon.Question))
            {
                try
                {
                    for (int i = 0; i < 10; i++)
                    {
                        ...
                        listaPersonajes.Push(unPersonaje); //Se agrega el objeto a la lista
        ...
        private void btnEliminar_Click(object sender, EventArgs e)
        {
            if (ValidarOperacion("¿Está seguro de querer eliminar este personaje?", MessageBoxIcon.Warning))
            {
                try
                {
                    ...
                    listaPersonajes.Pop(unPersonajeEliminado);
                    AgregarDatagridView();
                    LimpiarControles();
                }
                catch (Exception ex)
        ...
        private void btnEliminarTop_Click(object sender, EventArgs e)
        {
            if (ValidarOperacion("¿Está seguro de querer eliminar el Top?", MessageBoxIcon.Warning))
            {
                ...
                {
                    Personaje unPersonajeEliminado = new Personaje();
                    MessageBox.Show(listaPersonajes.Pop().ToString(), "Personaje Eliminado", MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
                    AgregarDatagridView();
                    LimpiarControles();
                }
                catch (Exception ex)
